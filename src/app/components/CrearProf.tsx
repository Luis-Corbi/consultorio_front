...
const CrearProfesionalForm: React.FC<CrearPacienteFormProps> = ({ onClose, onCreate }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
          ...
          const selectedSpeciality = specialities.find(s => s.id === parseInt(speciality));
    
          const newUser: Omit<User, "id" | "roles"> & { roles: number[] } = {
            username,
            password,
            name,
            lastname,
            ...
            speciality: selectedSpeciality ? { id: selectedSpeciality.id, name: selectedSpeciality.name } : { id: 0, name: 'Sin especialidad' }, // Valor por defecto
            notes,
            roles: [1],  // Ajusta los roles según sea necesario
            color
          };
